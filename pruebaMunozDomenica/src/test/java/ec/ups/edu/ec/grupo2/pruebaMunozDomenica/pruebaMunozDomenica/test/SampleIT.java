package ec.ups.edu.ec.grupo2.pruebaMunozDomenica.pruebaMunozDomenica.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/pruebaMunozDomenica.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "pruebaMunozDomenica.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("ec.ups.edu.ec.grupo2.pruebaMunozDomenica.pruebaMunozDomenica.test");
